...
def sharpening_median_filter(spectra: np.ndarray) -> np.ndarray:
    ...
    )  # The index of the middle element of the sliding window
    median_lower_bound_idx = mid_point_idx - (
        FILTER_SIZE // 2
    )  # The lower bound index of the median filter
    median_upper_bound_idx = (
        mid_point_idx + (FILTER_SIZE // 2) + 1
    )  # The upper bound index of the median filter
    ...
    filtered_values = np.where(
        (sliding_view[:, :, :, mid_point_idx] < lower_fence)
        | (sliding_view[:, :, :, mid_point_idx] > upper_fence),
        np.median(
            sliding_view[:, :, :, median_lower_bound_idx:median_upper_bound_idx], axis=3
        ),
...
def moving_median_filter(spectra: np.ndarray) -> np.ndarray:
    ...
    filtered_spectra = spectra.copy()
    filtered_spectra[:, :, lower_bound_idx:-upper_bound_idx] = spectral_median[
...
def moving_mean_filter(spectra: np.ndarray) -> np.ndarray:
    ...
    spectral_mean = scipy.ndimage.convolve1d(
        spectra, weights=np.ones(FILTER_SIZE) / FILTER_SIZE, axis=-1
    )

    filtered_spectra = spectra.copy()
    filtered_spectra[:, :, lower_bound_idx:-upper_bound_idx] = spectral_mean[
        :, :, lower_bound_idx:-upper_bound_idx
    ]
    return filtered_spectra
...
def cotcat_denoise(
    ...
    if breakpoint_idx == -0:
        filtered_spectra = moving_mean_filter(
            moving_median_filter(sharpening_median_filter(spectra))
        )
    elif len(wavelengths) - breakpoint_idx < 11 or breakpoint_idx < 11:
        raise ValueError(
            "The breakpoint is too close to the end of the spectra to apply CoTCAT denoising."
        )
    else:
        filtered_spectra = np.append(
            moving_mean_filter(
                moving_median_filter(
                    sharpening_median_filter(spectra[:, :, :breakpoint_idx])
                )
            ),
            (
                moving_mean_filter(
