...
def test_load_images_from_shortcode():
    ...
    pytest.raises(ValueError, postprocessing.load_image_from_shortcode, good_mineral_sample, bad_data_dir) # Check Value Error raised if directory for image doesn't exist
    pytest.raises(ValueError, postprocessing.load_image_from_shortcode, bad_mineral_sample, good_data_dir) # Check Value Error raised if image file doesn't exist

def test_calculate_pixel_blandness(crism_image):
    im_shape = image_shape(crism_image)
    spectra = crism_image["IF"][:, BAND_MASK]
    blandness_scores = postprocessing.calculate_pixel_blandness(spectra, im_shape=im_shape)
    assert type(blandness_scores) == np.ndarray
    assert blandness_scores.shape == im_shape
    assert blandness_scores[0,0] == -np.inf # Corner pixels are always bad pixels, so should be -np.inf
    pytest.raises(ValueError, postprocessing.calculate_pixel_blandness, spectra[:100], im_shape)
    pytest.raises(TypeError, postprocessing.calculate_pixel_blandness, im_shape, im_shape)   
