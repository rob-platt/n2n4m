...
CRISM_ML_DATA_DIR = os.path.join(PACKAGE_DIR, "data", "CRISM_ML")

@pytest.fixture
def crism_image():
    return load_image("tests/test_io/3561F/ATU0003561F_01_IF168L_TRR3.img")
...
def test_calculate_pixel_blandness(crism_image):
    ...
    blandness_scores = postprocessing.calculate_pixel_blandness(
        spectra, im_shape=im_shape, train_set_dir = CRISM_ML_DATA_DIR
    )
    ...
    pytest.raises(
        ValueError, postprocessing.calculate_pixel_blandness, spectra[:100], im_shape, CRISM_ML_DATA_DIR
    )
    pytest.raises(
        TypeError, postprocessing.calculate_pixel_blandness, im_shape, im_shape, CRISM_ML_DATA_DIR
    )
