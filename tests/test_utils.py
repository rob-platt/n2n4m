...
def test_label_list_to_string():
    test_df = pd.DataFrame({"Pixel_Class": [[1], [2], [3]], "Image_Name": ["test1", "test2", "test3"]})
...
def test_convert_coordinates_to_xy():
    test_df = pd.DataFrame({"Coordinates": [[1, 2], [3, 4], [5, 6]], "Image_Name": ["test1", "test2", "test3"]})
...
def test_convert_xy_to_coordinates():
    test_df = pd.DataFrame({"x": [1, 3, 5], "y": [2, 4, 6], "Image_Name": ["test1", "test2", "test3"]})
    test_df_converted = utils.convert_xy_to_coordinates(test_df)

    assert type(test_df_converted) == pd.DataFrame
    assert test_df_converted.shape == (3, 2) # removed one column as x, y now coordinates
