...
# Denoises the images using N2N4M before calculating the blandness of each pixel.
# The data is saved to the data/extracted_mineral_pixel_data dir as bland_pixels_to_match_mineral_pixels_n2n4m_denoised.json
...
TMP_DIR = join(
    DATA_DIR, "tmp_bland_pixel_finder_n2n4m_data"
...
for image_id in mineral_pixel_data["Image_Name"].unique():
    # Find the image file
    image_folder_list = os.listdir(IMAGE_DATA_DIR)
    image_folder = [folder for folder in image_folder_list if image_id in folder]
    if len(image_folder) == 0 or len(image_folder) > 1:
        raise ValueError(
            f"Image folder not found or multiple folders found for shortcode {image_id}."
        )
    image_folder = os.path.join(IMAGE_DATA_DIR, image_folder[0])
    image_filename = [
    ...
    if len(image_filename) == 0 or len(image_filename) > 1:
        raise ValueError(
            f"Image file not found or multiple files found for shortcode {image_id}."
        )
...
# Save the dataframe
bland_pixel_data.to_json(
    join(OUTPUT_DIR, "bland_pixels_to_match_mineral_pixels_n2n4m_denoised.json")
)
