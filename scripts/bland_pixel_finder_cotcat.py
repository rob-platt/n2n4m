...
# Denoises the images using CoTCAT [1] before calculating the blandness of each pixel.
# The data is saved to the data/extracted_mineral_pixel_data dir as bland_pixels_to_match_mineral_pixels_cotcat_denoised.json
# References:
...
from n2n4m.preprocessing import load_dataset, impute_bad_values_in_image
...
TMP_DIR = join(
    DATA_DIR, "tmp_bland_pixel_finder_cotcat_data"
...
for image_id in mineral_pixel_data["Image_Name"].unique():
    ...
    image_array, _ = impute_bad_values_in_image(image_array)  # Remove any bad values
    image_array = cotcat_denoise(
        image_array, wavelengths=PLEBANI_WAVELENGTHS
    )  # Denoise the image using CoTCAT [1]
...
bland_pixel_data.to_json(
    join(OUTPUT_DIR, "bland_pixels_to_match_mineral_pixels_cotcat_denoised.json")
)
